...
// Randomly generated variabe's range (fully defined in code in comments)
  ...
  const double Q2_max = 4.1;        // Maximum in Q^2; GeV^2
  const double Q2_min = 0.5;        // Minimum in Q^2; Don't go too low (must be > 0.2 for sure); GeV^2
  const double P1_xyz_mean = 0.;
  const double P1_xyz_sigma = 0.2;
...
// Define Random variables
  ...
  P1_vec[0] = myRand.Gaus(P1_xyz_mean, P1_xyz_sigma);  // initial nucleon momentum 'x'; Random; Typical: between [-0.4,0.4] GeV
  P1_vec[1] = myRand.Gaus(P1_xyz_mean, P1_xyz_sigma);  // initial nucleon momentum 'y'; Random; Typical: between [-0.4,0.4] GeV
  P1_vec[2] = myRand.Gaus(P1_xyz_mean, P1_xyz_sigma);  // initial nucleon momentum 'z'; Random; Typical: between [-0.4,0.4] GeV
...
// Creating weighting function: uniform
  ...
  double normalize_range = P1x_prob*P1y_prob*P1z_prob/((xb_max-xb_min)*(Q2_max-Q2_min)*(phi_max-phi_min));  // (range of random variables)
